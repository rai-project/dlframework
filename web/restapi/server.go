...
// NewServer creates a new api dlframework server but does not configure it
func NewServer(api *operations.DlframeworkAPI) *Server {
	s := new(Server)
...
// Server for the dlframework API
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/dlframework.sock"`
...
func (s *Server) SetAPI(api *operations.DlframeworkAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
